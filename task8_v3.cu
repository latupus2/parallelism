...
int main(int argc, char *argv[])
{
    ...
    desc.add_options()
        ("help,h", "help")
        ("size", po::value<int>(&N)->default_value(128), "grid size")
    ...
    calculate_step<<<blocks, BLOCK_SIZE, 0, stream>>>(N, d_in, d_out, d_err);
    ...
    while (error > eps && iter < max_iters)
    {
        ...
        reduce_global<<<1, BLOCK_SIZE, 0, stream>>>(blocks, d_err);
        cudaMemcpyAsync(&error, d_err, sizeof(double), cudaMemcpyDeviceToHost, stream);
        cudaStreamSynchronize(stream);
